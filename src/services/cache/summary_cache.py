...
class SummaryCache:
    ...
    async def get(self, video_id: str, language: str, summary_type: str) -> Optional[Dict[str, Any]]:
        if not self._db:
            await self.init()
        async with self._db.execute("SELECT summary_json, created_at FROM summary_cache WHERE video_id = ? AND language = ? AND summary_type = ?", (video_id, language, summary_type)) as cursor:
    ...
    async def set(self, video_id: str, language: str, summary_type: str, value: Dict[str, Any]) -> None:
        if not self._db:
            await self.init()
        await self._db.execute("INSERT OR REPLACE INTO summary_cache (video_id, language, summary_type, summary_json, created_at) VALUES (?, ?, ?, ?, ?)", (video_id, language, summary_type, json.dumps(value, ensure_ascii=False), time.time()))
        await self._db.commit()

    async def delete(self, video_id: str, language: str, summary_type: str) -> None:
        if not self._db:
            await self.init()
        await self._db.execute("DELETE FROM summary_cache WHERE video_id = ? AND language = ? AND summary_type = ?", (video_id, language, summary_type))
        await self._db.commit()